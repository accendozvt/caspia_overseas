      ...
      {eyebrow && (
        <p
          className={`text-xs md:text-sm font-bold uppercase tracking-[0.2em] mb-3 ${
            // `light` means the section sits on a dark background, where the brand
            // orange already passes. On light sections it needs the darker step.
...
export function WhatsAppCTA({
  ...
  return (
    <a
      ...
      className={`inline-flex items-center gap-2 px-6 py-3 rounded-full font-bold text-white bg-orange-strong hover:brightness-110 shadow-lg shadow-orange-strong/25 transition-all ${className}`}
    >
  ...
  return (
    <details className="group bg-white rounded-xl shadow-sm ring-1 ring-black/5 open:ring-accent/40 transition-shadow hover:shadow-md">
      <summary className="flex items-start justify-between gap-4 cursor-pointer list-none px-5 py-4 font-semibold text-secondary [&::-webkit-details-marker]:hidden">
        <span>
          <span className="text-accent-ink">{n}.</span> {item.q}
        </span>
        <span
          className="mt-1 text-accent-ink transition-transform group-open:rotate-45"
          aria-hidden
        >
